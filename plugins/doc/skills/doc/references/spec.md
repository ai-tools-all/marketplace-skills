# Spec template — folded in from `to-spec`

A **spec is just a plan**. There is no separate spec doc type: run `/doc plan <idx> "<title>"`
and fill the created plan file using the template below.
...
3. Write the spec into the plan file using the template below.
