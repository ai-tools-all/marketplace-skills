...
# Render filename
FILENAME="$(render_name "$(cfg naming.checkpoint 2>/dev/null || echo '{NN}-{datetime}-{description}.md')" "$NUM" "$SLUG")"

CKPT_PATH="$CKPT_DIR/$FILENAME"

emit_frontmatter checkpoint "$DESC" "$(expt_topic "$EXPT_NAME")" "experiments/$EXPT_NAME" > "$CKPT_PATH"
